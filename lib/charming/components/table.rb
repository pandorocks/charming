...
module Charming
  module Components
    class Table < Component
      ...
      # Top border + header text. The separator line that tty-table emits
      # between header and rows is stripped in #render so that body row N
      # lives at y = HEADER_HEIGHT + N.
      HEADER_HEIGHT = 2
      ...
      def handle_key(event)
        ...
        case Charming.key_of(event).to_sym
      ...
      def compact_layout(lines)
        ...
        highlighted = body.each_with_index.map do |line, i|
          (i == @selected_index) ? "\e[7m#{line}\e[m" : line
        end
